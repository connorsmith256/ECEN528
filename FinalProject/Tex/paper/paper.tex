...
A paginated journal article \cite{test1}, an enumerated
journal article \cite{test1}, a reference to an entire issue \cite{test1},
a monograph (whole book) \cite{test1}, a monograph/whole book in a series (see 2a in spec. document)
\cite{test1}, a divisible-book such as an anthology or compilation \cite{test1}
followed by the same example, however we only output the series if the volume number is given
\cite{test1} (so Editor00a's series should NOT be present since it has no vol. no.),
a chapter in a divisible book \cite{test1}, a chapter in a divisible book
in a series \cite{test1}, a multi-volume work as book \cite{test1},
an article in a proceedings (of a conference, symposium, workshop for example)
(paginated proceedings article) \cite{test1}, a proceedings article
with all possible elements \cite{test1}, an example of an enumerated
proceedings article \cite{test1},
an informally published work \cite{test1}, a doctoral dissertation \cite{test1},
a master's thesis: \cite{test1}, an online document / world wide web resource \cite{test1}, \cite{test1},
\cite{test1}, a video game (Case 1) \cite{test1} and (Case 2) \cite{test1}
and \cite{test1} and (Case 3) a patent \cite{test1},
work accepted for publication \cite{test1}, 'YYYYb'-test for prolific author
\cite{test1} and \cite{test1}. Other cites might contain
'duplicate' DOI and URLs (some SIAM articles) \cite{test1}.
Boris / Barbara Beeton: multi-volume works as books
\cite{test1} and \cite{test1}.
...
% Acknowledgments
\begin{acks}
TODO The author would like to thank Dr. Penry for providing another great semester.
\end{acks}
